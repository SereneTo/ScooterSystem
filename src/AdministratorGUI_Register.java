/**
 *  This part matches to the part which the administrator clicks the button which name is "Register".
 *  The administrator should input the user information correctly so that he or she can successfully create a student account.
 * @author Group 44
 * @version 3.1
 */
...
public class AdministratorGUI_Register extends CheckInput implements ActionListener {
	...
	/**
	 * The action listener, which contains the checking method to judge if the administrator inputs the correct form of each information.
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (eventSource == register)
		{
			...
			{
				JOptionPane.showOptionDialog(null,"Your name should be started with a upper case. e.g.:Shenlinger","Please enter again",JOptionPane.DEFAULT_OPTION,
						JOptionPane.WARNING_MESSAGE,null,options,options[0]);
			}
			else if(isAllWords(name_text.getText())) {
				JOptionPane.showOptionDialog(null,"Your name should not contain a digit. e.g.:Shenlinger","Please enter again",JOptionPane.DEFAULT_OPTION,
						JOptionPane.WARNING_MESSAGE,null,options,options[0]);
			}
			else if((isNumeric(id_text.getText()))||(id_text.getText().length()!=9)) {
				JOptionPane.showOptionDialog(null,"Your id should be a 9-digit number. e.g.:161197197","Please enter again",JOptionPane.DEFAULT_OPTION,
						JOptionPane.WARNING_MESSAGE,null,options,options[0]);
			}
			else if (validateEmail(email_text.getText())) {
				JOptionPane.showOptionDialog(null,"Your email format is not valid. e.g.:161197197@163.com","Please enter again",JOptionPane.DEFAULT_OPTION,
						JOptionPane.WARNING_MESSAGE,null,options,options[0]);
			}
			...
			else if (new File(id_text.getText()).exists()==false)//check if the user ID is in database
			{
